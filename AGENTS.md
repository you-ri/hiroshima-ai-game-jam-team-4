...
- 現状は**スターター**。`scenes/main.tscn` は**仮のプレースホルダ**（ラベルを出すだけ）で、差し替え前提。ゲーム本体はこれから作る。
...
```
scenes/     画面・ステージ（main.tscn ＝ 仮のプレースホルダ, ui/title.tscn）
actors/     再利用する実体。1 機能 1 フォルダで .tscn と .gd を同居させる（actors/player/player.tscn + player.gd）
...
| 設定 | 値 | 意味 |
|---|---|---|
| `run/main_scene` | `res://scenes/main.tscn` | 仮のシーン。差し替えるときはこの設定も合わせる（消すと `Can't run project` で全員が動かせなくなる） |
...
- **OpenCode** — このファイル（`AGENTS.md`）を読む。追加設定は不要。
